...
function openRotorHelpSheet() {
  openSheet((sheet, close) => {
    ...
    body.innerHTML = `
      ...
      <h4 style="color:var(--bad);margin:18px 0 4px;letter-spacing:1px">🚧 ЗАМЕТКА — новый метод (не реализован, не забыть)</h4>
      <p>Текущий расчёт lead_mil выше и текст «в стороне от 3 часов» — устарел,
      путает горизонталь с вертикалью. Заменить на метод ниже.</p>
      <p><b>Точка прицеливания = «3 часа»</b> при вращении ПО ЧАСОВОЙ — это точка,
      где гонг пересекает горизонтальный уровень вала и прячется (правая сторона
      круга, если появление слева/уход направо). Именно тут гонг движется строго
      вертикально — поэтому упреждение нужно откладывать по вертикали, а не
      по горизонтали.</p>
      <p><b>Система координат (зафиксировано):</b> ось Y — плюс ВВЕРХ от
      перекрестия, минус ВНИЗ. Ось X — плюс ВПРАВО от перекрестия, минус ВЛЕВО.</p>
      <p><b>Формула метки</b> (относительно точки «3 часа»):<br>
      ω = 2π/T · &nbsp; φ = ω×(TOF + время_реакции) · &nbsp; Δx = R×(cosφ−1) ·
      &nbsp; Δy = R×sinφ, затем в mil: ÷дистанция×1000.<br>
      В координатах выше: метка на <b>+Δy_mil (вверх)</b> и <b>−Δx_mil (влево,
      т.к. при часовой Δx отрицателен)</b> от точки «3 часа».</p>
      <p><b>Как стрелять (проверенный трюк):</b> докрутить барабанчик ВВЕРХ на
      Δy_mil <i>сверх</i> обычной поправки на дистанцию (не вместо!). Удерживать
      точку «3 часа» меткой «−Δy_mil» на ёлочке (не центром). Смотреть на
      горизонтальную линию перекрестия — она встанет на нужной высоте. Жать
      спуск в момент пересечения линии гонгом (пересечение будет на Δx_mil от
      вертикали — проверка правильности момента).</p>
      <p class="muted">Числовой пример (V₀=830, 175gr, G7=0.261, R=1.5м, T=5с,
      N=4, D=400м, стандартная атмо, реакция 0.2с): TOF=0.562с (реальный расчёт
      солвером) → φ=54.8° → метка: докрутка +3.07 mil (≈31 клик), держать
      «−3.07» на ёлочке, пересечение на 1.59 mil влево от вертикали.</p>
    `;
