...
route('/reticle/:id', async ({ id }) => {
  ...
  const refRow = el('div', { class: 'row', style: 'margin-top:8px' },
    numInput('refMilV', 'Ссыл. точка — от центра, mil (+D)', cal?.p1_mil_v ?? 5, { step: '0.1' }),
  ...
  canvas.addEventListener('click', (e) => {
    ...
    if (mode === 'center') {
      ...
      calStateInfo.textContent = `Ссылочная точка: (${fmt(x_nat,0)}, ${fmt(y_nat,0)}) px = (${fmt(Math.abs(d.refMilH),1)} mil ${d.refMilH < 0 ? 'L' : 'R'}, ${fmt(Math.abs(d.refMilV),1)} mil ${d.refMilV < 0 ? 'U' : 'D'}).`;
    }
    drawCanvas();
  });
...
function openRotorHelpSheet() {
  openSheet((sheet, close) => {
    ...
    body.innerHTML = `
      ...
      <p>«Вращение (вид от стрелка)» переключает по часовой/против — это меняет, в какую сторону (L или R) будет пересечение. «Реакция+спуск» — твоя личная задержка между «увидел момент» и «пуля вышла из ствола» (обычно 0.15–0.3 с) — расчёт добавляет её к времени полёта пули, чтобы упреждение учитывало и её.</p>
