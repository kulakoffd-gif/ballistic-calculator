...
function openRotorHelpSheet() {
  openSheet((sheet, close) => {
    ...
    body.innerHTML = `
      ...
      <p><b>2. Докрути барабанчик</b> на «Итоговую накрутку» из расчёта (это обычная поправка на дистанцию + доп. смещение метода «3 часа» — они складываются, а не заменяют друг друга).</p>
      <p><b>3. Наведи метку «ёлочки»</b> (значение «Держать метку на ёлочке» — это метка ПО ВЕРТИКАЛИ, на оси Y, прямо на вертикальной линии перекрестия, ниже центра, число отрицательное) точно на опорную точку у вала из шага 1. За счёт докрутки из шага 2 твоя ОСНОВНАЯ горизонтальная линия перекрестия физически встанет выше — на нужной высоте для встречи с гонгом позже.</p>
      <p><b>4. Жди и стреляй.</b> Гонг приближается снизу/сбоку по дуге. Стреляй в момент, когда ЦЕНТР гонга пересечёт твою (уже поднятую докруткой) горизонтальную линию перекрестия. Это пересечение произойдёт на расчётном смещении ПО ГОРИЗОНТАЛИ (ось X, значение «Пересечение линии») от вертикальной линии — отдельно наводить туда не нужно, это просто проверка, что всё сошлось как надо.</p>
      ...
      <p>«Вращение (вид от стрелка)» переключает по часовой/против — это меняет, в какую сторону (влево/вправо) будет пересечение. «Реакция+спуск» — твоя личная задержка между «увидел момент» и «пуля вышла из ствола» (обычно 0.15–0.3 с) — расчёт добавляет её к времени полёта пули, чтобы упреждение учитывало и её.</p>
...
route('/moving-target', async () => {
  ...
  function renderRotorResult(row) {
    ...
    resultCard.appendChild(el('div', { class: 'kv' },
      ...
      el('div', { class: 'k' }, 'Держать метку на ёлочке (ось Y, вертикаль)'), el('div', { class: 'v' }, `${fmt(holdMark, 2)} mil от центра`),
      el('div', { class: 'k' }, 'Пересечение линии (ось X, горизонталь)'), el('div', { class: 'v accent' }, `${fmt(Math.abs(dx_mil), 2)} mil ${dx_mil < 0 ? 'влево' : 'вправо'}`)
    ));
    resultCard.appendChild(el('div', { class: 'kv' },
      el('div', { class: 'k' }, 'Время полёта пули'), el('div', { class: 'v' }, fmt(tof, 2) + ' с'),
    ...
    resultCard.appendChild(el('div', { class: 'banner accent' },
      ...
      `2) Держи метку «${fmt(holdMark, 2)}» на ёлочке (это метка ПО ВЕРТИКАЛИ, на оси Y, на самой вертикальной линии перекрестия) в точке у вала, где гонг ТОЛЬКО НАЧИНАЕТ касаться вала своим нижним краем (первый момент касания — гонг ещё виден полностью). ` +
      `3) Жми спуск, когда гонг пересечёт ГОРИЗОНТАЛЬНУЮ линию перекрестия — это случится на ${fmt(Math.abs(dx_mil), 2)} mil ${dx_mil < 0 ? 'левее' : 'правее'} вертикали (это смещение ПО ГОРИЗОНТАЛИ, по оси X — просто сверься, что всё сошлось, отдельно наводить сюда не нужно).`));
